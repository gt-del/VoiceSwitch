...
2. 检查 `README.md` 是否需要同步。
3. 检查 `AGENTS.md` 中是否存在与该设置项相关的长期约束。
4. 检查设置说明、调试文案、validation rule 说明是否一致。
5. 检查这次变更是否影响旧配置兼容性或现有用户迁移。
6. 明确哪些内容属于 README，哪些属于 AGENTS，避免把 AGENTS 写成设置说明书。

## 输出工件

- 使用 `templates/settings-sync-checklist.md`
